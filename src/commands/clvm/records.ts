...
import {
  ...
  pruneRuntimeRawArchive,
  writeJsonStateAtomic,
  writeRuntimeRawArchive,
  type RuntimeRawArchiveOptions,
...
export function clvmStatePath(): string {
  return join(codexToolsCacheDir(), "clvm-state.json");
}

export function clvmHistoryPath(): string {
  return join(codexToolsCacheDir(), "clvm-history.jsonl");
}

export function clvmRawDir(): string {
  return join(codexToolsCacheDir(), "clvm-raw");
}

export async function recordClvmSample(
  source: SampleSource,
...
export async function recordClvmFailure(source: SampleSource, config: RuntimeConfig, failure: MonitorFailure, dedupe?: ClvmRuntimeRecordDedupe): Promise<void> {
  const record = buildClvmFailureRecord(source, config, failure);
  await writeClvmRuntimeRecord(record, dedupe);
}

export function buildMonitorFailure(error: unknown, retry?: ClvmRetryState, includeRaw = false): MonitorFailure {
  return {
...
export function buildRetryState(attempt: number, intervalMs: number, now = Date.now()): ClvmRetryState {
  return {
    attempt,
    intervalMs,
...
export function recordClosedConnections(closedHistory: ClosedConnectionEntry[], closedConnections: ConnectionEntry[]): void {
...
export function toJsonResult(result: MonitorResult): Record<string, unknown> {
  return {
    ok: true,
    status: "ok",
...
export function toJsonFailure(failure: MonitorFailure): Record<string, unknown> {
  return {
    ok: false,
    status: "unavailable",
...
async function writeClvmRawPayload(raw: unknown) {
...
function toClvmStateRecord(record: ClvmRuntimeRecord): ClvmStateRecord {
  ...
  if (state.ok) {
    return {
      ...state,
      result: toJsonResult(state.result as MonitorResult),
    };
  }
  return state;
}
...
function buildClvmFailureRecord(source: SampleSource, config: RuntimeConfig, failure: MonitorFailure): ClvmRuntimeRecord {
  return {
    version: clvmStateVersion,
    ok: false,
    status: "unavailable",
...
function sanitizedCloseFailures(closeFailures: Array<Record<string, unknown>>): Array<Record<string, unknown>> {
