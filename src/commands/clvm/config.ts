...
import type { ClvmConfig, ClvmConfigFile, CommandOptions, RuntimeConfig } from "./types.js";
...
export function requireResolvedClvmConfig(config: ClvmConfigFile, path: string): ClvmConfig {
  return {
...
export function parseCloseZeroForSeconds(value: string): number | null {
  if (value === "off") {
    return null;
  }
...
export function parseRawArchiveMode(value: string): boolean {
  if (value === "on") {
    return true;
  }
  if (value === "off") {
...
export function parseNonNegativeNumber(value: unknown, name: string): number {
