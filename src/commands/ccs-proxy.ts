...
import { assertProfile, readProfiles, writeProfiles, type ProfilesFile, type ProxySearchConfig } from "../lib/profiles.js";
...
type ProxyStateReset = {
  previousSchema: number | null;
...
type ProxyRoute =
  | { kind: "control"; endpoint: "health" | "reroute" | "cancel" | "capture" }
  | { kind: "upstream_api"; endpointClass: ProxyEndpointClass | null; policyManaged: boolean }
  | { kind: "invalid" };
...
type ProxyCaptureStatus = {
  active: boolean;
  task: Omit<ProxyCaptureTask, "directory"> | null;
  directory: string | null;
};

type ProxyCaptureCancelResult = {
  cancelled: boolean;
  task: ProxyCaptureStatus["task"];
  directory: string | null;
};

type ProxyCaptureRequestContext = {
  taskId: string;
  sequence: number;
  requestId: string;
  sessionId: string;
...
const PROXY_DEFAULT_STATUS_RETRY: ProxyStatusRetry = {
  enabled: false,
...
const PROXY_TABLE_STATE_WIDTH = 14;
...
const PROXY_ACTIVE_OVERVIEW_TABLE_COLUMNS: TableColumn[] = [
...
const PROXY_ACTIVE_TOKEN_TABLE_COLUMNS: TableColumn[] = [
...
const PROXY_ACTIVE_COST_TABLE_COLUMNS: TableColumn[] = [
  ...PROXY_ACTIVE_OVERVIEW_TABLE_COLUMNS.slice(0, 3),
...
export async function readProxyState(stateRoot: string = process.env.CCS_PROXY_STATE_ROOT || path.join(codexToolsCacheDir(), "proxy")): Promise<ProxyState | null> {
  ...
  return raw ? normalizeProxyState(raw as ProxyState) : null;
}

export async function proxyStateExists(stateRoot: string = process.env.CCS_PROXY_STATE_ROOT || path.join(codexToolsCacheDir(), "proxy")): Promise<boolean> {
  return (await readRawProxyState(stateRoot)) !== null;
}

async function readRawProxyState(stateRoot: string): Promise<Record<string, unknown> | null> {
...
async function resetIncompatibleProxyState(options: ProxyOptions): Promise<ProxyStateReset | null> {
  const raw = await readRawProxyState(options.stateRoot);
  if (!raw || raw.state_schema_version === PROXY_STATE_SCHEMA_VERSION) {
    return null;
  }

  const installedAt = requireProxyField(raw, "installed_at", "proxy.json", isNonEmptyString, "non-empty string") as string;
  const providerName = requireProxyField(raw, "provider_name", "proxy.json", isNonEmptyString, "non-empty string") as string;
  const originalBaseUrl = requireProxyField(raw, "original_base_url", "proxy.json", isNonEmptyString, "non-empty string") as string;
  const storedProxyBaseUrl = requireProxyField(raw, "proxy_base_url", "proxy.json", isNonEmptyString, "non-empty string") as string;
  const listenHost = requireProxyField(raw, "listen_host", "proxy.json", isNonEmptyString, "non-empty string") as string;
  const listenPort = requireProxyField(raw, "listen_port", "proxy.json", isProxyPort, "TCP port integer") as number;
  const backupPath = requireProxyField(raw, "backup_path", "proxy.json", (field) => typeof field === "string", "string") as string;
...
function isProxyPort(value: unknown): boolean {
...
function rerouteUrl(state: ProxyState): string {
  return new URL(REROUTE_PATH, state.proxy_base_url).toString();
}

function cancelUrl(state: ProxyState, target?: string): string {
  const url = new URL(CANCEL_PATH, state.proxy_base_url);
  if (target) url.searchParams.set("target", target);
  return url.toString();
}

function captureUrl(state: ProxyState): string {
  return new URL(CAPTURE_PATH, state.proxy_base_url).toString();
}

function captureRootPath(stateRoot: string): string {
...
function sanitizeCaptureHeaders(headers: IncomingMessage["headers"] | Record<string, string>): Record<string, string> {
...
function publicCaptureStatus(task: ProxyCaptureTask | null): ProxyCaptureStatus {
  if (!task) return { active: false, task: null, directory: null };
  const { directory, ...publicTask } = task;
  return { active: true, task: publicTask, directory };
}

function startCaptureRequest(
  task: ProxyCaptureTask | null,
  sessionId: string | null,
  requestId: string,
  method: string,
  url: string,
  ...
  requestBody: Buffer,
): ProxyCaptureRequestContext | null {
  if (!task || task.claimed >= task.requested || !sessionId || !sessionId.startsWith(task.target)) return null;
...
async function writeCaptureRecord(
  task: ProxyCaptureTask,
  capture: ProxyCaptureRequestContext,
  completedAt: string,
  completion: ProxyCaptureCompletion,
): Promise<void> {
...
async function readProxyHealth(state: ProxyState, timeoutMs = PROXY_HEALTH_TIMEOUT_MS): Promise<ProxyHealth> {
  try {
    ...
    const payload = await response.json().catch(() => null) as { status?: unknown; pid?: unknown; version?: unknown; protocol?: unknown; mode?: unknown; status_retry_enabled?: unknown } | null;
    return {
...
async function waitForProxyHealth(state: ProxyState, stateRoot: string): Promise<ProxyHealth> {
...
function parseProxyRetrySummary(value: unknown, source: string): void {
  const summary = requireProxyObject(value, source);
  for (const field of ["total", "reasoning_guard", "upstream_capacity", "http_429", "http_503", "timeout", "transport"] as const) {
...
function normalizeProxyState(state: ProxyState | null): ProxyState | null {
  ...
  requireProxyField(
    ...
    "proxy.json",
    (field) => field === PROXY_STATE_SCHEMA_VERSION,
    `number ${PROXY_STATE_SCHEMA_VERSION}`,
  );
  return {
    ...state,
    mode: normalizeProxyMode((state as Record<string, unknown>).mode) ?? PROXY_DEFAULT_MODE,
    status_retry: parseProxyStatusRetry((state as Record<string, unknown>).status_retry, "proxy.json.status_retry"),
...
function parseProxyStatusRetry(value: unknown, source: string): ProxyStatusRetry {
  ...
  const totalWindowMs = requireProxyField(raw, "total_window_ms", source, isPositiveProxyTimerMs, "positive timer-range integer") as number;
  const backoffBaseMs = requireProxyField(raw, "backoff_base_ms", source, isPositiveProxyTimerMs, "positive timer-range integer") as number;
  const backoffMaxMs = requireProxyField(raw, "backoff_max_ms", source, isPositiveProxyTimerMs, "positive timer-range integer") as number;
...
function isPositiveProxyTimerMs(value: unknown): boolean {
  return Number.isInteger(value) && Number(value) > 0 && Number(value) <= PROXY_TIMER_MAX_MS;
}
...
function isProxyInspectionMode(mode: ProxyMode): boolean {
  return mode === PROXY_MODE_INTERCEPT || mode === PROXY_MODE_RECOVERY;
}

function proxyStatusRetryEnabled(state: Pick<ProxyState, "status_retry">): boolean {
...
function formatProxyHistoryRequest(
  record: ProxyRequestRecord,
  nowMs: number,
...
function formatProxyActiveRequest(
  record: ProxyRequestRecord,
  nowMs: number,
...
function formatProxyProvider(upstream: string | null): string {
  return upstream
    ? colorName(truncateVisible(upstream, PROXY_TABLE_PROVIDER_WIDTH, ""))
    : textDim("-");
}

function formatProxyRoute(record: ProxyRequestRecord): string {
...
function extractSessionId(requestJson: unknown, turnMetadata: Record<string, unknown> | null): string | null {
  return jsonStringAt(turnMetadata, ["session_id"])
...
type ProxyForwardCallbacks = {
  ...
  resolveRetryUpstream?: () => Promise<ProxyUpstream>;
  onStatusRetryWait?: (wait: ProxyStatusRetryWait) => (() => void);
};
...
type ProxyCancelRequest = ProxyRerouteRequest;

type ProxyCancelStatus = {
  target: string;
  requests: ProxyCancelRequest[];
};

type ProxyCancelResult = {
  cancelled: string[];
  skipped: string[];
};

type ProxyStatusRetryWaitEntry = ProxyCancelRequest & {
...
type ProxyRerouteStatus = {
  mode: ProxyMode;
  profile: string;
  requests: ProxyRerouteRequest[];
};

type ProxyRerouteResult = {
  profile: string;
  rerouted: string[];
  skipped: string[];
};
...
async function proxyThroughActiveUpstreamWithStats(
  ...
  statusRetryConfig?: ProxyStatusRetry,
  ...
  while (true) {
    ...
    if (statusRetryConfig && isRetryableUpstreamStatus(status)) {
      const attempt = currentProxyAttemptRecord(attemptState)!;
      const trigger: ProxyPolicyTrigger = status === 429 ? "http_429" : "http_503";
      ...
      if (retryAfter.kind !== "exceeds_limit" && remainingMs > 0 && delayMs <= remainingMs) {
        ...
        const abortFromClient = (): void => waitAbort.abort("client");
        if (callbacks.signal?.aborted) abortFromClient();
        else callbacks.signal?.addEventListener("abort", abortFromClient, { once: true });
        const unregisterWait = callbacks.onStatusRetryWait?.({
          ...
          cancel: () => waitAbort.abort("cancel"),
        });
        let waitResult: "ready" | "aborted" | "deadline";
        try {
        ...
        if (waitResult === "aborted" && waitAbort.signal.reason !== "reroute") {
          ...
          throw new ProxyResponseWriteError(waitAbort.signal.reason === "cancel" ? "retry cancelled by ccs proxy" : "client closed response before upstream retry", 499, 0);
        }
...
function isRetryableUpstreamStatus(status: number): status is 429 | 503 | 520 {
  return status === 429 || status === 503 || status === 520;
}

async function proxyThroughActiveUpstreamStatusRetry(
  request: IncomingMessage,
  initialUpstream: ProxyUpstream,
  body: Buffer,
  gatewayRequestId: string,
  attemptRecords: ProxyAttemptRecord[],
  config: ProxyStatusRetry,
  callbacks: ProxyForwardCallbacks,
): Promise<ProxyOutcome> {
  const attemptState: ProxyAttemptState = {
    gatewayRequestId,
...
function proxyFailureSummaryFromBufferedPayload(status: number, buffer: Buffer, contentType: string, headers?: Headers): ProxyFailureSummary | null {
  if (status < 400) {
    return null;
  }
...
function proxyHttpFailureSummary(status: number, headers?: Headers): ProxyFailureSummary {
...
function appendProxyFailureDiagnostics(summary: ProxyFailureSummary, headers?: Headers): ProxyFailureSummary {
...
function proxyFailureDiagnostics(headers?: Headers): string | null {
...
function writeProxyJsonErrorResponse(
  res: ServerResponse,
  status: number,
  message: string,
  onHeadersWritten?: () => void,
  responseObserver?: ProxyClientResponseObserver,
): boolean {
...
async function writeResponse(
  ...
  const scanner = conversion?.needsConversion
    ...
    : existingScanner ?? (inspectStream && isStreamContentType(`${finalResponse.headers.get("content-type") || ""}`)
      ? new ProxySseScanner(endpointClass)
      : null);
  return writeReadableResponse(res, Readable.fromWeb(finalResponse.body as never), scanner, modelObserver, onResponseBytes, responseObserver);
}
...
async function writeReadableResponse(
  ...
  return new Promise<number>((resolve, reject) => {
    ...
    stream.on("error", (error) => finish(new ProxyResponseWriteError(
      error instanceof ProxyLatencyTimeoutError || error instanceof ProxyInspectionLimitError || error instanceof ProtocolConversionError
        ? error.message
        : `upstream stream terminated: ${error.message}`,
      502,
      responseBytes,
...
function formatProxyPolicySummary(records: ProxyRequestRecord[]): string | null {
  ...
  const categories = [
    ["capacity", totals.upstream_capacity],
    ["429", totals.http_429],
    ["503", totals.http_503],
    ["reasoning", totals.reasoning_guard],
    ["timeout", totals.timeout],
    ["transport", totals.transport],
  ] as const;
  return [
    `policy retries=${colorCount(String(retries))}`,
    ...categories.filter(([, count]) => count > 0).map(([label, count]) => `${label}=${colorCount(String(count))}`),
  ].join(" ");
}
...
function formatProxyReasoningSummary(metrics: ProxyMetrics): string | null {
  const reasoningCounts = formatGroupedProxyReasoningTokenCounts(metrics.reasoning_token_counts);
  const recoveryCounts = formatProxyContinuationRecoveryCounts(proxyContinuationRecoveryCounts(metrics.recent_requests));
  if (reasoningCounts.length === 0 && recoveryCounts.length === 0) {
    return null;
  }
  return [
    "reasoning",
...
function formatProxyPolicyLines(state: ProxyState | null, metrics: ProxyMetrics): string[] {
  if (!state) {
    return [];
  }
  const lines: string[] = [];
  ...
  if (mode === PROXY_MODE_PASSTHROUGH) return lines;
...
export function proxyRequestTableColumns(view: ProxyView): TableColumn[] {
  if (view === "tokens") return PROXY_ACTIVE_TOKEN_TABLE_COLUMNS;
  if (view === "cost") return PROXY_ACTIVE_COST_TABLE_COLUMNS;
...
function resolveProxyHistoryRenderCount(metrics: ProxyMetrics, options: ProxyOptions, state: ProxyState | null): number {
  ...
  let usedLines = 1
    ...
    + (options.watch ? 1 : 0);
  let count = 0;
  const recordsPerGroup = new Map<string, number>();
...
function formatProxyHistoryRows(
  ...
  for (const group of proxyHistoryGroups(visibleRecords)) {
    ...
    const completeRows = group
      ...
      .map((record) => styleTableRow(
        formatProxyHistoryRequest(record, now.getTime(), priceCache),
        textDim,
      ));
...
function proxyHistoryGroups(records: ProxyRequestRecord[]): ProxyRequestRecord[][] {
  const groups = new Map<string, ProxyRequestRecord[]>();
  for (const record of records) {
...
function takeProxyHistoryGroups(records: ProxyRequestRecord[], count: number): ProxyRequestRecord[] {
  const groupKeys = new Set<string>();
  const selected: ProxyRequestRecord[] = [];
  for (const record of records) {
    const key = proxyHistoryGroupKey(record);
...
function formatProxyHistoryEarlier(records: ProxyRequestRecord[]): string {
...
async function readProxyRequestTail(stateRoot: string, groupCount: number): Promise<ProxyRequestRecord[]> {
  if (groupCount <= 0) {
    return [];
  }
  let file;
...
async function proxyRerouteRequest<T>(state: ProxyState, init?: RequestInit): Promise<T> {
...
async function proxyCancelRequest<T>(state: ProxyState, target: string, init?: RequestInit): Promise<T> {
  const response = await fetch(cancelUrl(state, init?.method === "GET" ? target : undefined), {
    headers: { accept: "application/json", "content-type": "application/json" },
    signal: AbortSignal.timeout(PROXY_HEALTH_TIMEOUT_MS),
    ...init,
  });
  const payload = await response.json().catch(() => null) as { error?: unknown } | null;
  if (!response.ok) {
    throw new Error(typeof payload?.error === "string" ? payload.error : `proxy cancel failed with HTTP ${response.status}`);
  }
  return payload as T;
}

async function proxyCaptureRequest<T>(state: ProxyState, init?: RequestInit): Promise<T> {
...
async function proxyCaptureCancelRequest(state: ProxyState): Promise<ProxyCaptureCancelResult> {
  return proxyCaptureRequest<ProxyCaptureCancelResult>(state, { method: "DELETE" });
}

async function proxyCaptureStatus(options: ProxyOptions): Promise<{ state: ProxyState; status: ProxyCaptureStatus }> {
  const state = await readProxyState(options.stateRoot);
  if (!state) throw new Error(`proxy state file was not found: ${statePath(options.stateRoot)}`);
  assertProxyHealthRuntime(await readProxyHealth(state));
  return { state, status: await proxyCaptureRequest<ProxyCaptureStatus>(state) };
}

async function buildProxyReroutePlan(options: ProxyOptions): Promise<{ state: ProxyState; status: ProxyRerouteStatus }> {
  const state = await readProxyState(options.stateRoot);
  if (!state) throw new Error(`proxy state file was not found: ${statePath(options.stateRoot)}`);
  assertProxyHealthRuntime(await readProxyHealth(state));
  return { state, status: await proxyRerouteRequest<ProxyRerouteStatus>(state) };
}

async function applyProxyReroutePlan(
  plan: { state: ProxyState; status: ProxyRerouteStatus },
): Promise<ProxyRerouteResult> {
  return proxyRerouteRequest<ProxyRerouteResult>(plan.state, {
    method: "POST",
    body: JSON.stringify({
...
async function buildProxyCancelPlan(options: ProxyOptions, target: string): Promise<{ state: ProxyState; status: ProxyCancelStatus }> {
  const state = await readProxyState(options.stateRoot);
  if (!state) throw new Error(`proxy state file was not found: ${statePath(options.stateRoot)}`);
  assertProxyHealthRuntime(await readProxyHealth(state));
  return { state, status: await proxyCancelRequest<ProxyCancelStatus>(state, target, { method: "GET" }) };
}

async function applyProxyCancelPlan(
  plan: { state: ProxyState; status: ProxyCancelStatus },
): Promise<ProxyCancelResult> {
  return proxyCancelRequest<ProxyCancelResult>(plan.state, plan.status.target, {
    method: "POST",
    body: JSON.stringify({ request_ids: plan.status.requests.map((request) => request.request_id) }),
  });
}
...
export async function setProxyMode(options: ProxyOptions, mode: ProxyMode, retryEnabled?: boolean): Promise<ProxyModeChangeResult> {
  const state = await readProxyState(options.stateRoot);
  if (!state) {
    throw new Error(`proxy state file was not found: ${statePath(options.stateRoot)}`);
  }
...
async function serveProxy(options: ProxyOptions): Promise<void> {
  ...
  let closing = false;
  const statusRetryWaits = new Map<string, ProxyStatusRetryWaitEntry>();
  let captureTask: ProxyCaptureTask | null = null;
      ...
      try {
        ...
        if (route.kind === "control") {
          ...
          if (route.endpoint === "reroute" || route.endpoint === "cancel") {
            ...
            if (route.endpoint === "reroute" && !proxyStatusRetryEnabled(currentState)) {
              res.writeHead(409, { "content-type": "application/json; charset=utf-8" });
            ...
            if (route.endpoint === "cancel") {
              if (method === "GET") {
                ...
                const payload: ProxyCancelStatus = {
                  target: selector,
                  requests: matches.map(({ session_id: _sessionId, pinned: _pinned, wake: _wake, cancel: _cancel, ...request }) => request),
                };
                res.writeHead(200, { "content-type": "application/json; charset=utf-8" });
              ...
              const requestIds = payload && typeof payload === "object" && !Array.isArray(payload)
                ? (payload as Record<string, unknown>).request_ids
                : null;
              if (!Array.isArray(requestIds) || !requestIds.every((id) => typeof id === "string")) {
                res.writeHead(409, { "content-type": "application/json; charset=utf-8" });
                res.end(JSON.stringify({ error: "proxy cancel preview is stale; run ccs proxy cancel again" }));
                return;
              }
              const cancelled: string[] = [];
              const skipped: string[] = [];
              for (const requestId of [...new Set(requestIds)]) {
                const wait = statusRetryWaits.get(requestId);
                if (!wait) {
                  skipped.push(requestId);
                  continue;
                }
                statusRetryWaits.delete(requestId);
                wait.cancel();
                cancelled.push(requestId);
              }
              const result: ProxyCancelResult = { cancelled, skipped };
              res.writeHead(200, { "content-type": "application/json; charset=utf-8" });
              res.end(JSON.stringify(result));
              return;
            }
            if (method === "GET") {
              const payload: ProxyRerouteStatus = {
                mode: currentState.mode,
                profile,
                requests: [...statusRetryWaits.values()]
                  .filter((wait) => !wait.pinned)
                  .map(({ session_id: _sessionId, pinned: _pinned, cancel: _cancel, wake: _wake, ...request }) => request)
            ...
            const result: ProxyRerouteResult = { profile, rerouted, skipped };
            res.writeHead(200, { "content-type": "application/json; charset=utf-8" });
            res.end(JSON.stringify(result));
            return;
          }
        ...
        const abortDownstream = (): void => {
          if (!downstreamAbort.signal.aborted) downstreamAbort.abort("client");
        };
        ...
        req.socket.once("close", () => {
          if (!responseFinished) abortDownstream();
        });
        ...
        let captureOwner: ProxyCaptureTask | null = null;
        let captureRequest: ProxyCaptureRequestContext | null = null;
        const captureResponseObserver: ProxyClientResponseObserver = {
        ...
        try {
          ...
          captureRequest = startCaptureRequest(
            captureOwner, sessionId, activeRecord.id, method, `${url.pathname}${url.search}`, req.headers, body,
          );
          if (captureOwner?.error) {
            ...
            if (captureTask?.id === captureOwner.id) captureTask = null;
            captureOwner = null;
          }
          const profiles = await readProfiles();
          ...
          activeRecord.session = sessionId ? shortSessionId(sessionId) : null;
          ...
          const passthroughCallbacks: ProxyForwardCallbacks = {
            ...
            onStatusRetryWait: (wait) => {
              const entry: ProxyStatusRetryWaitEntry = {
                ...
                pinned: requestedProfile.length > 0,
                wake: wait.wake,
                cancel: wait.cancel,
              };
              statusRetryWaits.set(wait.requestId, entry);
              return () => {
                if (statusRetryWaits.get(wait.requestId) === entry) statusRetryWaits.delete(wait.requestId);
              };
            },
          ...
          const outcome = !route.policyManaged || (mode === PROXY_MODE_PASSTHROUGH && !retryEnabled)
            ...
            : mode === PROXY_MODE_PASSTHROUGH
              ? await proxyThroughActiveUpstreamStatusRetry(
                req,
                upstreamProfile,
                body,
                activeRecord.id,
                attemptRecords,
                requestState.status_retry,
                passthroughCallbacks,
              )
              : await proxyThroughActiveUpstreamWithStats(
                ...
                isProxyInspectionMode(mode) ? guardedCallbacks : passthroughCallbacks,
                retryEnabled ? requestState.status_retry : undefined,
              );
          ...
          if (responseControlLost && pendingAttempt && error instanceof ProxyResponseWriteError) {
            ...
            completeLastPendingProxyAttemptRecord(
              attemptRecords,
              conversionFailureStage ? "route_conversion_failed" : status === 499 ? "client_aborted" : upstreamStreamTerminated ? "upstream_stream_terminated" : "gateway_error",
              { failureSummary },
            );
          }
...
function parseProxyUserMode(value: string | undefined): ProxyMode | "retry" {
  if (value === PROXY_MODE_PASSTHROUGH) {
    return PROXY_MODE_PASSTHROUGH;
  }
  if (value === PROXY_MODE_INTERCEPT) {
...
function parseProxyStatusRetryConfig(args: string[], enabled: boolean): ProxyStatusRetry {
  if (args.length !== 3) {
...
function printProxyStatusRetry(value: ProxyStatusRetry): void {
...
function proxySearchConfig(profiles: ProfilesFile): ProxySearchConfig {
  return profiles.proxy?.search ?? { enabled: false };
}

function printProxySearchStatus(profiles: ProfilesFile): void {
...
function assertSearchProfile(profiles: ProfilesFile, name: string): void {
  const profile = profiles.profiles?.[name];
  if (!profile) {
...
async function applyProxySearchConfig(expectedProfiles: ProfilesFile, nextSearch: ProxySearchConfig): Promise<void> {
...
export async function runProxyCommand(args: string[], options: ProxyOptions): Promise<void> {
  ...
  if (reset) {
    printKeyValue(
      "state:",
      textYellow(`reset schema ${reset.previousSchema === null ? "legacy" : reset.previousSchema} -> ${reset.currentSchema}`),
      6,
    );
  ...
  if (command === "reroute") {
    ...
    for (const request of plan.status.requests) {
      ...
      printKeyValue(
        "request:",
        `${request.request_id} session=${request.session ?? "-"} upstream=${request.upstream} status=${request.status} attempt=${request.attempt} wait=${formatDurationMs(waitedMs)}`,
        9,
      );
    }
  ...
  if (command === "cancel") {
    if (rest.length !== 1 || !rest[0]) {
      throw new Error("ccs proxy cancel requires TARGET");
    }
    const plan = await buildProxyCancelPlan(options, rest[0]);
    printKeyValue("target:", rest[0], 9);
    printKeyValue("eligible:", String(plan.status.requests.length), 9);
    for (const request of plan.status.requests) {
      const waitedMs = Math.max(0, Date.now() - Date.parse(request.waiting_since));
      printKeyValue(
    ...
    printKeyValue("confirm:", "no requests are cancelled unless you type yes", 9);
    if (!(await confirmApply())) return;
  ...
  if (command === "capture") {
    ...
    const { state, status } = await proxyCaptureStatus(options);
    if (status.active) throw new Error("proxy capture already has an active task");
    const started = await proxyCaptureRequest<ProxyCaptureStatus>(state, {
      method: "POST",
  ...
  if (command === "mode") {
    if (rest.length === 0) {
      ...
      if (state) printKeyValue("retry:", proxyStatusRetryEnabled(state) ? textGreen("enabled") : textDim("disabled"), 5);
      return;
    }
  ...
  if (command === "config") {
    ...
    if (rest[0] === "retry") printProxyStatusRetry(statusRetry);
    else printProxyLatencyGuard(latencyGuard);
    printKeyValue("note:", "no changes are written unless you type yes", 5);
    if (!(await confirmApply())) return;
