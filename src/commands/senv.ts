...
async function resolveArgs(args: Args): Promise<ResolvedArgs> {
...
function printPlan(target: string, current: string, next: string, summary: Summary): void {
...
function shouldMaskEnvKey(key: string): boolean {
  return /(?:KEY|TOKEN|SECRET|PASSWORD|PASS|AUTH|CREDENTIAL)/i.test(key);
}

function redactEnvDiffContent(content: string): string {
...
function printDiff(target: string, current: string, next: string): void {
  if (current === next) {
    return;
  }
  ...
  const patch = createTwoFilesPatch(
    `current/${target}`,
    `next/${target}`,
    redactEnvDiffContent(current),
    redactEnvDiffContent(next),
    "",
    "",
    { context: 3 },
  );
