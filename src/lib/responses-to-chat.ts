...
export type ConversionFailureStage = "request" | "response_json" | "response_stream";

export class ProtocolConversionError extends Error {
  constructor(readonly stage: ConversionFailureStage, message: string) {
    super(message);
...
export type ResponsesChatToolContext = {
  byChatName: Map<string, { name: string; namespace?: string }>;
};

export function createResponsesChatToolContext(): ResponsesChatToolContext {
  return { byChatName: new Map() };
}

export function buildResponsesChatToolContext(request: Pick<ResponsesRequest, "tools">): ResponsesChatToolContext {
  const context = createResponsesChatToolContext();
  for (const tool of request.tools ?? []) convertTool(tool, context);
  return context;
}

export function responsesToChat(request: ResponsesRequest, toolContext = createResponsesChatToolContext()): Json {
  const messages: Json[] = [];
  ...
  if (request.stream) result.stream_options = { include_usage: true };
  if (request.temperature !== undefined) result.temperature = request.temperature;
  if (request.max_output_tokens !== undefined) result.max_completion_tokens = request.max_output_tokens;
  if (request.parallel_tool_calls !== undefined) result.parallel_tool_calls = request.parallel_tool_calls;
  if (request.service_tier !== undefined) result.service_tier = request.service_tier;
  if (request.store !== undefined) result.store = request.store;
  if (request.reasoning && typeof request.reasoning.effort === "string") {
    result.reasoning_effort = request.reasoning.effort;
  }
  if (request.tools?.length) result.tools = request.tools.flatMap((tool) => convertTool(tool, toolContext));
  if (request.tool_choice !== undefined) result.tool_choice = convertToolChoice(request.tool_choice);
  const responseFormat = convertTextFormat(request.text);
  if (responseFormat) result.response_format = responseFormat;
  return result;
}

function appendInputItem(messages: Json[], item: Json, toolContext: ResponsesChatToolContext): void {
...
function convertMessageContent(content: unknown, role: string): string | Json[] {
  ...
  const parts = content.map((part): Json => {
    if (!isJson(part)) throw new Error("Responses message content parts must be objects");
...
function convertTool(tool: Json, toolContext: ResponsesChatToolContext): Json[] {
  if (tool.type === "web_search" || tool.type === "web_search_preview") return [];
  if (tool.type === "namespace") {
    const namespace = requireString(tool.name, "namespace.name");
    if (!Array.isArray(tool.tools)) throw new Error("namespace.tools must be an array");
    return tool.tools.flatMap((child) => {
      if (!isJson(child) || child.type !== "function") throw new Error(`unsupported namespace child tool: ${isJson(child) ? String(child.type) : typeof child}`);
      return convertFunctionTool(child, toolContext, namespace);
    });
  }
  if (tool.type !== "function") throw new Error(`unsupported Responses tool: ${String(tool.type)}`);
  return convertFunctionTool(tool, toolContext);
}

function convertFunctionTool(tool: Json, toolContext: ResponsesChatToolContext, namespace?: string): Json[] {
...
function convertTextFormat(text: Json | undefined): Json | null {
  if (!text || !isJson(text.format)) return null;
...
export function chatToResponses(chat: Json, toolContext = createResponsesChatToolContext()): Json {
  const choice = Array.isArray(chat.choices) && isJson(chat.choices[0]) ? chat.choices[0] : null;
  if (!choice || !isJson(choice.message)) throw new Error("Chat Completions response requires choices[0].message");
...
function buildResponseOutput(message: Json, toolContext: ResponsesChatToolContext): Json[] {
  const output: Json[] = [];
  if (typeof message.content === "string" || typeof message.refusal === "string") {
    const content: Json[] = [];
    if (typeof message.content === "string") content.push({ type: "output_text", text: message.content, annotations: [] });
  ...
  if (Array.isArray(message.tool_calls)) {
    for (const rawCall of message.tool_calls) {
      if (!isJson(rawCall) || !isJson(rawCall.function)) continue;
...
function responseObject(input: ResponseObjectInput): Json {
  return {
    id: input.id,
    object: "response",
...
export class ChatToResponsesSseTransform extends Transform {
  ...
  #outputs: Json[] = [];
  #message: Json | null = null;
  #messageText = "";
  #refusalText = "";
  #messageParts: Array<"output_text" | "refusal"> = [];
  #tools = new Map<number, Json>();

  constructor(private readonly toolContext = createResponsesChatToolContext()) {
    super();
  }

  endAfterUpstreamDisconnect(): boolean {
    if (this.#completed || this.#finishReason === null) return false;
    this.end();
    return true;
  }
  ...
  override _transform(chunk: Buffer, _encoding: BufferEncoding, callback: (error?: Error | null) => void): void {
    try {
      ...
      for (const event of events) this.#convertEvent(event);
      callback();
    } catch (error) {
  ...
  override _flush(callback: (error?: Error | null) => void): void {
    try {
      ...
      if (this.#buffer.trim()) this.#convertEvent(this.#buffer);
      if (!this.#completed && this.#finishReason !== null) this.#complete();
      callback();
    } catch (error) {
      callback(error instanceof ProtocolConversionError
        ? error
  ...
  #convertEvent(event: string): void {
    ...
    if (!data) return;
    if (data === "[DONE]") {
      if (this.#finishReason === null) throw new Error("Chat Completions stream ended before finish_reason");
      this.#complete();
      return;
    }
    ...
    if (isJson(chunk.usage)) this.#usage = convertUsage(chunk.usage);
    this.#start();

    const choice = Array.isArray(chunk.choices) && isJson(chunk.choices[0]) ? chunk.choices[0] : null;
    if (!choice) return;
    if (choice.finish_reason !== null && choice.finish_reason !== undefined) this.#finishReason = choice.finish_reason;
    if (!isJson(choice.delta)) return;
    const delta = choice.delta;
    if (typeof delta.content === "string" && delta.content) this.#appendText(delta.content);
    if (typeof delta.refusal === "string" && delta.refusal) this.#appendRefusal(delta.refusal);
    if (Array.isArray(delta.tool_calls)) {
      for (const rawTool of delta.tool_calls) if (isJson(rawTool)) this.#appendTool(rawTool);
    }
  }
  ...
  #ensureMessagePart(type: "output_text" | "refusal"): { item: Json; contentIndex: number } {
    if (!this.#message) {
      const item = { id: itemId("msg"), type: "message", status: "in_progress", role: "assistant", content: [] as Json[] };
  ...
  #appendTool(rawTool: Json): void {
    ...
    if (!item) {
      ...
      if (typeof rawTool.id === "string") item.call_id = rawTool.id;
      if (typeof fn.name === "string") item.name = `${item.name as string}${fn.name}`;
    }
    if (typeof fn.arguments === "string" && fn.arguments) {
      item.arguments = `${item.arguments as string}${fn.arguments}`;
...
function chatToolName(name: string, namespace: string | undefined, toolContext: ResponsesChatToolContext): string {
