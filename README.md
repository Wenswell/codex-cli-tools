...
- Active and history records use the complete request schema `5` contract and one shared row formatter. State snapshots and JSONL history accept schema `5` records with every required field. Upgrading from an earlier request schema requires a clean proxy reinstall: restore the configured provider URL from the active profile, remove the proxy state directory, then run `ccs proxy install`. The reinstall creates current metrics and history files. Fixed columns are right-aligned; the final `error` column uses the remaining width. Sessions retain stable colors across active and history rows. Active overview rows expose observed upstream, model, `reas./code`, latency, and byte facts. The `size` cell shows response bytes received so far and updates for every forwarded upstream chunk; a new retry clears attempt-scoped response facts until the new attempt reports them.
