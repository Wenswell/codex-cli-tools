...
- Proxy metrics are recomputed from `proxy.json.metrics.recent_requests`, so request statistics cover the newest 100 completed requests in the state file plus the current active count. Status counters use exact HTTP status codes, such as `200`, `404`, `499`, and `502`. `status total` is the sum of those exact counters. Reasoning counters use the same completed-request basis: each completed request with final explicit `reasoning_tokens` contributes one count. Guard retry actions keep per-attempt `reasoning_tokens` in request history and do not change `reasoning total`. `max` is the largest final request reasoning token value. Reasoning summary renders non-zero fixed groups `0`, `516`, `1034`, `1552`, and `other`; when all groups are zero, only `total` and `max` are shown. Guarded values are red, `0` is orange, and `other` is green. Latency shows `last`, `avg`, `min`, and `max` for the same state window. `ccs proxy restore` removes the state file, and the next install starts a new statistics window.
