...
- `ccs proxy mode` prints the active response mode and independent status-retry state. Every change previews and requires exact `yes`. `passthrough` forwards responses without inspection. `ccs proxy mode retry [on|off]` controls 429/503 retry without changing the response mode; `ccs proxy mode passthrough retry` selects the transparent-plus-retry combination. `recovery` enables continuation recovery for eligible streaming Responses guard hits; `intercept` uses ordinary guard retry.
...
- Active and history records use request schema `8`; proxy health uses protocol `8`. State snapshots and JSONL history require the current contract. `retry_summary` separates HTTP 429 and 503 retries. JSONL `attempt_records` identify each dispatched forwarding attempt and store protocol conversion, actual upstream endpoint, conversion failure stage, status retry triggers, and delays without prompt or response bodies.
