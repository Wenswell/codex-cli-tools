...
The no-argument proxy snapshot uses the same compact runtime, traffic, policy, and request-table layout as `watch`. Use `ccs proxy --help` for file locations, status options, modes, commands, and configuration syntax.
...
- `ccs proxy` without arguments ensures the installed runtime and prints one compact snapshot. `ccs proxy watch` reads state and health without starting, upgrading, or replacing the runtime. Both show a runtime line, traffic and latency summaries, active/non-zero policy details, and one request table; default output omits file paths, disabled policies, zero policy counters, latency minimum/maximum, and command hints.
- Every request view begins with `state session time up model` and ends with `result`. `state` is `live` or `done`; live rows precede dim completed rows under one table header. Overview adds `api dur. size`, where `api` shows `R→C` for Responses-to-Chat-Completions conversion; tokens adds `input output cached`; cost adds `input$ output$ cached$ total$`. Live `dur.` is elapsed time and completed `dur.` is total request time. Equal visible session ids keep the same color across rows, while the first 11 distinct visible ids use different ANSI 256 palette colors. The 10-cell model column shows the available actual model, colors equal request/upstream models green and differing actual upstream models red, then abbreviates `gpt-` to `o` and truncates.
...
- The live request table places current requests before completed requests. Current rows include upstream SSE buffering before client headers are written. Streamed byte progress persists the first observation immediately, coalesces later updates to at most once per second, and writes the exact final count at completion. Completed, failed, and fully streamed model API responses become dim `done` rows. `proxy.json.metrics.recent_requests` keeps the newest 100 completed model API requests as a compact status snapshot; `proxy-requests.jsonl` keeps bounded completed model API request history in completion order. Compact state history is written before the complete JSONL record append, so JSONL append failures are recorded in `proxy.log` after the client response state is settled. Default status rendering reads history from `proxy.json`. TTY output uses all rows remaining after the compact status, live rows, table header, and watch footer; non-TTY output uses 5 completed rows, and `--history N` overrides both. When `--history N` exceeds the snapshot length, status rendering reads the tail of `proxy-requests.jsonl`.
...
- Proxy metrics are recomputed from `proxy.json.metrics.recent_requests`, so request statistics cover the newest 100 completed model API requests plus the current live count. Exact status-code events and upstream hits appear on the traffic line. Policy summaries aggregate `retry_summary` across the complete recent-request window, independent of rendered rows and `--history`, and omit zero categories. Enabled deadlines and retry configuration remain visible. Reasoning groups and last/average latency use the same state window. `ccs proxy restore` removes the state file, and the next install starts a new statistics window.
