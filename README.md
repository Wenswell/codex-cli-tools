...
- Every view begins with `session time up model` and ends with `result`. Overview adds `reas./code dur. size`; tokens adds `input output cached`; cost adds `input$ output$ cached$ total$`. Active `dur.` is elapsed time and history `dur.` is total request time. Equal visible session ids keep the same color across active and history rows, while the first six distinct visible ids use different bright palette colors. The 10-cell model column shows the available actual model, colors equal request/upstream models green and differing actual upstream models red, then abbreviates `gpt-` to `o` and truncates.
