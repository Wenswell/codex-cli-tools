...
- `ccs proxy` without arguments ensures the installed runtime and prints one compact snapshot. `ccs proxy watch` reads state and health without starting, upgrading, or replacing the runtime. Both show mode, last/average latency, status counts, active/non-zero policy details, a full active table, and grouped history. Healthy output omits URL, PID, version, protocol, file paths, disabled policies, zero policy counters, latency minimum/maximum, and command hints.
- `active` and `history` remain separate. Active rows use `session time up model` plus view-specific measurements and `result`. Consecutive history rows with the same session, upstream, actual model, and API conversion share one context line; their table retains only `time status`, view-specific measurements, and `result`. Equal consecutive status/result values render once and remain blank until they change. Equal visible session ids keep the same color across sections, while the first 11 distinct visible ids use different ANSI 256 palette colors.
