...
- `ccs`, `ccs status`, `ccs toggle`, and `ccs PROFILE` print a `usage:` line with local time. `ccs list` fetches usage for all profiles in parallel and prints it as the last column. Usage is fetched from `BASE_URL/v1/usage` with the profile API key; failures print `usage: HH:MM:SS unavailable` or `unavailable`, and missing keys print `usage: HH:MM:SS skipped` or `skipped`.
