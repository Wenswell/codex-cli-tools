...
- Proxy metrics count completed requests by exact HTTP status code, such as `200`, `404`, `499`, and `502`. `status total` is the sum of those exact counters. Proxy metrics also count every observed `reasoning_tokens` event from completed requests, including guard retry actions and accepted final response values. A final local `502 reasoning_guard_triggered` records the last guarded value through its `return_status_502` action. `reasoning total` is the sum of observed reasoning-token events, and `max` is the largest observed value. Reasoning summary renders non-zero fixed groups `0`, `516`, `1034`, `1552`, and `other`; when all groups are zero, only `total` and `max` are shown. Guarded values are red, `0` is orange, and `other` is green. Latency shows `last`, `avg`, `min`, and `max`.
