...
- The live proxy view separates `active` and `history`. `active` contains HTTP requests currently being processed by the proxy. Completed, failed, and fully streamed responses move to `history`. Both tables use `session time up code ms size req_model up_model path` columns, right-align every column, and show up to 5 rows.
- For `/v1/chat/completions`, `/chat/completions`, `/v1/responses`, and `/responses`, proxy metrics record request model and upstream model metadata when the JSON or SSE payload contains it. Missing model fields are stored as `null` and render as red `[unknown]`; matching request/upstream models render as dim `[same]`; differing upstream models render in orange. SSE model extraction stops after the first valid model value and keeps the forwarded response bytes unchanged.
