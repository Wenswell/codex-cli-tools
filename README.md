...
- Retry attempts render as a yellow number after the upstream name, such as `input3`. Request rows with guard actions prefix `error` with bracketed local-action values; HTTP status entries are yellow and reasoning-token entries are red, such as `[516 516 516] reasoning_guard_triggered ...`.
