...
ccs proxy mode [recovery|intercept]
ccs proxy install|restore|stop|serve
ccs cost
ccs cost daily
ccs cost weekly
ccs cost monthly
...
- `ccs proxy stop` switches mode to `passthrough` and keeps the proxy URL active. Passthrough disables only reasoning guard and continuation behavior; Capacity, HTTP 429, transport, and enabled latency policies remain independently evaluated.
...
- Transport-level `TypeError: fetch failed` is retried once for the same upstream in every mode. Repeated transport failure returns `502` with error type `upstream_error` and code `upstream_fetch_failed`.
...
- `ccs proxy` without arguments prints runtime state, paths, summaries, active requests, and completed history once and exits. `--view overview|tokens|cost` selects the request table; overview is the default. `ccs proxy watch` refreshes the selected view in the terminal alternate screen and pins its current view, history visibility, and shortcut keys to the terminal's bottom row. Press `v` to cycle overview, tokens, and cost; press `t` to hide or show the history section; press `q` or `Ctrl-C` to exit cleanly. History starts visible, and hiding it skips history rendering and explicit JSONL tail reads until it is shown again.
...
- When latency policy is enabled, the absolute total deadline starts with the first upstream fetch and spans retries and waits. The first-progress deadline restarts for each real attempt and accepts only non-empty output text, a final answer, or a tool call as progress. Before client headers, timeout and response-inspection failures return `502`; after client headers, the proxy aborts upstream and disconnects the client because response status is already committed.
- SSE inspection is incremental across arbitrary chunks, mixed LF/CR/CRLF delimiters, and a BOM split across chunks. Accepted bytes are forwarded unchanged. A candidate SSE event larger than 1 MiB fails with `response_inspection_limit_exceeded` before headers or disconnects after headers.
...
- Active and history records use request schema `6`; proxy health uses protocol `5`. State snapshots and JSONL history require the current contract. JSONL `attempt_records` identify each real upstream dispatch and store progress timing, policy decision, retry delay/budget, timeout boundary, client/upstream status, and final result. `proxy.log` emits normalized `ccs_proxy_policy_trigger` and `ccs_proxy_attempt_completed` events without prompt or response bodies. Upgrading from an earlier schema requires a clean proxy reinstall. Fixed columns are right-aligned; the final `error` column uses the remaining width. Sessions retain stable colors across active and history rows. Active overview rows expose observed upstream, model, `reas./code`, latency, and byte facts. The `size` cell shows response bytes received so far and updates for every forwarded upstream chunk; a new retry clears attempt-scoped response facts until the new attempt reports them.
- For `/v1/chat/completions`, `/chat/completions`, `/v1/responses`, and `/responses`, proxy metrics record mode, request kind, request model, request reasoning effort, request service tier, client turn id, client request attempt, upstream model, upstream self-reported metadata, explicit usage, reasoning observations, response shape, retry summary, and timing. `latency_ms` is total proxy request latency, `upstream_wait_ms` is final-attempt time to upstream headers, and `client_ttfb_ms` is time to proxy response headers written to the Codex client. Compact records add one `usage_attempts` entry for every upstream attempt, including attempts without usage and passthrough attempts. Each entry preserves its token fields plus pricing model and tier attribution. Request records store `request_body_sha256`; prompt and response text stay outside runtime records.
