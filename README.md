...
When proxy mode is installed, use `ccs proxy search` to inspect alpha-search forwarding, `ccs proxy search set PROFILE` to select its provider, and `ccs proxy search up|down` to enable or disable it. The example above sends `/v1/alpha/search` to `ciii`; an explicit `cx run PROFILE` (or equivalent) profile remains higher priority.
...
- `ccs proxy` applies the active proxy mode only to `/responses`, `/v1/responses`, `/chat/completions`, and `/v1/chat/completions`. Other upstream API paths, including `/v1/alpha/search`, are transparently forwarded once and still enter request metrics. Health and reroute endpoints under `/__codex_proxy/*` are local control traffic; unknown local control paths return a local `404 unsupported_proxy_path` that identifies `ccs proxy` as the rejecting component and do not enter request metrics or history.
...
- `active` and `history` remain separate. Active rows use `session route state age size`; route combines a six-character provider with an eight-character normalized model, and state carries conversion or current retry facts. History shows each non-empty session once, using its newest route context, retains the newest five complete `time status dur. size result` events, then compresses older events to ordered `status/dur.` pairs. Requests without a session remain separate. Equal visible session ids keep the same color across sections, while the first 11 distinct visible ids use different ANSI 256 palette colors.
