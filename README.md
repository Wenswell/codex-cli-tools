...
- Active and history records use request schema `6`; proxy health uses protocol `5`. State snapshots and JSONL history require the current contract. JSONL `attempt_records` identify each real upstream dispatch and store progress timing, policy decision, retry delay/budget, timeout boundary, client/upstream status, and final result. `proxy.log` emits normalized `ccs_proxy_policy_trigger` and `ccs_proxy_attempt_completed` events without prompt or response bodies. Upgrading from an earlier schema requires a clean proxy reinstall. Fixed columns are right-aligned; the final `result` column uses the remaining width. Sessions retain stable colors across active and history rows. Active overview rows expose observed upstream, model, `reas./code`, duration, and byte facts. The `size` cell shows response bytes received so far and updates for every forwarded upstream chunk; a new retry clears attempt-scoped response facts until the new attempt reports them.
