...
- `ccs`, `ccs status`, `ccs toggle`, and `ccs PROFILE` print a `usage:` line with local time. Usage is fetched from `BASE_URL/v1/usage` with the profile API key; failures print `usage: HH:MM:SS unavailable`, and missing keys print `usage: HH:MM:SS skipped`.
