...
- The live proxy view separates `active` and `history`. `active` contains HTTP requests currently being processed by the proxy. Completed, failed, and fully streamed responses move to `history`. Both tables use `session time up code ms size req_model up_model path error` columns and show up to 5 rows. Fixed-width columns are right-aligned; the final `error` column is left-aligned.
