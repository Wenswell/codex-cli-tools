...
- `ccs proxy install` requires explicit `model_provider`, its existing `base_url`, a current profile, and absent proxy state. It previews the current and proxy URLs plus backup path, backs up the current config, starts and health-checks the proxy, then changes and verifies only that provider's `base_url`. Mode starts as `passthrough`; use `ccs proxy mode recovery` or `ccs proxy mode intercept` to enable intervention.
...
- `ccs proxy` without arguments prints runtime state, paths, summaries, active requests, and completed history once and exits. `--view overview|tokens|cost` selects the request table; overview is the default. `ccs proxy watch` refreshes the selected view in the terminal alternate screen. Press `v` to cycle overview, tokens, and cost; press `q` to exit cleanly.
- Every view begins with `session time up model` and ends with `error`. Overview adds `reas./code lat. size`; tokens adds `input_tokens output_tokens cached_input_tokens`; cost adds `input$ output$ cached$ total$`. The 10-cell model column shows the available actual model, colors equal request/upstream models green and differing actual upstream models red, then abbreviates `gpt-` to `o` and truncates.
- Token and cost columns aggregate every proxy-owned upstream attempt. A token total appears only when every attempt reports that field. Cost uses each attempt's exact upstream/request model and response/request/config service tier with local cached prices and profile overrides; it performs no network refresh. Missing facts remain visible as `-` or `missing`, and cached input above input tokens renders `invalid`.
...
- Active and history records use request schema `4` and one shared row formatter. Fixed columns are right-aligned; the final `error` column uses the remaining width. Sessions retain stable colors across active and history rows. Active overview rows expose observed upstream, model, `reas./code`, latency, and byte facts; a new retry clears attempt-scoped response facts until the new attempt reports them.
- For `/v1/chat/completions`, `/chat/completions`, `/v1/responses`, and `/responses`, proxy metrics record mode, request kind, request model, request reasoning effort, request service tier, client turn id, client request attempt, upstream model, upstream self-reported metadata, explicit usage, reasoning observations, response shape, retry summary, and timing. Compact records add one `usage_attempts` entry for every upstream attempt, including attempts without usage and passthrough attempts. Each entry preserves its token fields plus pricing model and tier attribution. Request records store `request_body_sha256`; prompt and response text stay outside runtime records.
