...
| `ccs cost` | `ccs-cost/`, `ccs-cost-derived.json` | aligned | `model-prices.json` lives under config. Derived runtime data lives under cache. Local reports read Codex session JSONL as source data and avoid storing prompt or response text. |
