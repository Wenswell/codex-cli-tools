...
- Add `cxr`, `cxxr`, and `cxxsr` as the remote counterparts of `cx`, `cxx`, and `cxxs`; each adds `--remote unix://` before any Codex subcommand.
...
3. Extend the existing launcher options with remote mode and add three small bin entrypoints.
...
- `cxr`, `cxxr`, and `cxxsr` preserve their base command flags and pass `--remote unix://` to Codex.
