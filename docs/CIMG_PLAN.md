...
- 默认输出到 `~/Pictures/cimg` 的时间戳 PNG；目录在确认生成后按需创建。`-o` / `--out` 可指定新文件，已有文件不覆盖。
- 生成前打印 profile、端点、模型、参数和输出路径；只有输入精确 `yes` 后才请求并写文件。
- 确认后立即显示 `generating` 状态；交互终端每 10 秒原位刷新已耗时，非交互输出只打印一次开始状态。接口不提供真实进度，因此不显示百分比。
- 生成期间按 `Ctrl-C` 主动中止 HTTP 请求，以退出码 `130` 结束，并记录规范化的取消终态。
...
- 用户取消属于 `failed`，错误码为 `canceled`，不新增日志事件类型。
...
- 无参数状态和帮助显示默认输出目录 `~/Pictures/cimg`，默认生成不会污染当前工作目录。
...
- 长请求期间持续显示已耗时；成功、失败或取消后清理动态状态行。
- `Ctrl-C` 中止请求、写入 `failed/canceled`，CLI 使用退出码 `130`。
