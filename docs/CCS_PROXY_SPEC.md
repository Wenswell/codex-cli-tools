...
When status retry is enabled, the total window starts immediately before the first upstream fetch. HTTP 429, 503, and transient gateway 520 responses are retryable while the configured total window allows another wait and dispatch; there is no fixed retry-count limit. A retryable response uses `Retry-After` seconds or HTTP date when the delay fits the configured window; a missing or invalid header uses full-jitter exponential backoff from `backoff_base_ms` through `backoff_max_ms`. A wait is abortable by the client: `req.aborted`, a premature client socket close, and an unfinished response close all stop the wait and prevent another upstream attempt. Before every retry dispatch, requests without `x-ccs-profile` reload `profiles.current`; explicit profile requests stay pinned. No attempt is dispatched after the window limit, and exhaustion returns the last original upstream status, headers, and body. In passthrough mode, the final response is forwarded without inspection.

`ccs proxy reroute` requires a healthy matching runtime with status retry enabled. `GET /__codex_proxy/reroute` returns the current profile and in-memory requests that are waiting after a retryable status (429, 503, or 520) and are not explicitly pinned. The CLI prints that plan and performs no action when the set is empty. After exact `yes`, `POST /__codex_proxy/reroute` verifies that the current profile still matches the preview, wakes the previewed requests that are still eligible, and reports rerouted and skipped IDs. Each awakened request cancels only its backoff wait, retains its original client connection, body, deadline, and counters, then resolves the current profile before its next dispatch. Fetching, forwarding, completed, and pinned requests are never registered for reroute.

`ccs proxy cancel TARGET` requires a healthy matching runtime. `TARGET` is a request ID or a unique session ID prefix (the status table shows the short session prefix). The CLI previews only requests currently waiting after a retryable status (429, 503, or 520), requires exact `yes`, and then aborts those waits. A session prefix that matches multiple sessions is rejected; request IDs always identify one wait. `GET /__codex_proxy/cancel` previews the matching waits and `POST /__codex_proxy/cancel` applies a previewed request-id set. Cancel applies to pinned and unpinned waits; fetching, streaming, completed, and non-waiting requests are not selected. A cancelled wait closes its upstream response body, prevents another attempt, and completes as status `499`, `final_action=client_aborted`, `failure_summary.code=proxy_retry_cancelled`, and error `retry cancelled by ccs proxy`.
...
Capacity, reasoning, transport retry, latency, and inspection policies are active only in `recovery` and `intercept`. Independent status retry uses HTTP 429/503/520 facts and may be enabled with any response mode. `passthrough` records status, duration, and forwarded bytes without response inspection.
...
- Policy lines: only active configuration and non-zero activity. `passthrough` has no policy line. `retry` shows its retry window/backoff and non-zero 429/503/520 retry counts. `intercept` and `recovery` show an enabled deadline, non-zero retry categories, and non-zero reasoning/recovery groups.
...
- Status retries reload the profile configuration before each later attempt and retain the original request pathname; `ccs proxy reroute` wakes eligible unpinned 429/503/520 waits without creating another client request.
