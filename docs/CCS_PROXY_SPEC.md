...
`ccs proxy cancel TARGET` requires a healthy matching runtime. `TARGET` is a request ID or a unique session ID prefix (the status table shows the short session prefix). The CLI previews only requests currently waiting after a retryable HTTP status (429, 503, or 520) or a Capacity body, requires exact `yes`, and then aborts those waits. A session prefix that matches multiple sessions is rejected; request IDs always identify one wait. `GET /__codex_proxy/cancel` previews the matching waits and `POST /__codex_proxy/cancel` applies a previewed request-id set. Cancel applies to pinned and unpinned waits; fetching, streaming, completed, and non-waiting requests are not selected. A cancelled wait closes its upstream response body, prevents another attempt, and completes as status `499`, `final_action=client_aborted`, `failure_summary.code=proxy_retry_cancelled`, and error `retry cancelled by ccs proxy`.
