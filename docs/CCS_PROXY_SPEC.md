...
- `model`: `request_model/upstream_model`, with `gpt-` prefixes shortened to `o` before terminal-width truncation. Missing values render dim `-`, matching upstream model renders dim `[same]`, and differing upstream model renders red.
