...
function buildUsageTopHistoryRawTotals(pointMap, names) {
    const times = [...new Set(names.flatMap((name) => ((pointMap.get(name) ?? []).map((point) => point.at.getTime()))))].sort((left, right) => left - right);
