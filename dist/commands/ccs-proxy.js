...
import { determineRouteConversion, convertRequestBody, convertResponseBody, createStreamingResponseConverter, rewriteUpstreamPath, } from "../lib/proxy-router.js";
import { ProtocolConversionError, buildResponsesChatToolContext } from "../lib/responses-to-chat.js";
...
function proxyRequestsPath(stateRoot) {
    return path.join(stateRoot, "proxy-requests.jsonl");
}
function proxyRuntimeLogPath(stateRoot) {
...
function cancelUrl(state, target) {
...
function proxyHealthRuntimeMatches(health) {
...
async function waitForProxyProcessExit(pid) {
    const deadline = Date.now() + PROXY_START_TIMEOUT_MS;
    while (Date.now() < deadline) {
        try {
            process.kill(pid, 0);
...
function isProxySearchPath(requestPath) {
...
function isConversionFailureStage(value) {
...
function resolveClientRequestAttempt(record, recentRequests) {
    ...
    return 1 + recentRequests.filter((candidate) => (candidate.client_turn_id === record.client_turn_id
        && candidate.request_body_sha256 === record.request_body_sha256)).length;
}
...
function incrementProxyUpstreamHitCountsForRecord(counts, record) {
    for (const action of record.guard_actions) {
...
function incrementProxyUpstreamHitCount(counts, upstream) {
    if (!upstream) {
        return;
    }
...
function formatProxyStatusLine(now, state, runtime, metrics) {
...
function formatProxyActiveState(record) {
    ...
    const compactSearchState = (value) => search ? value.replace(" x", "x") : value;
...
function proxyDisplayError(record) {
...
function proxyFailureSummaryDisplay(summary) {
    if (!summary) {
        return null;
    }
...
function formatProxyErrorPrefix(record) {
    const values = [
...
function formatProxyClientRequestAttemptPrefixValue(attempt) {
    return attempt > 1 ? textYellow(`client:${attempt}`) : "";
}
function formatProxyGuardActionPrefixValue(action, requestStatus) {
...
function proxyGuardActionDisplayStatus(action, requestStatus) {
...
async function logProxyRequestError(stateRoot, request, status, error) {
    await appendProxyJsonLine(proxyLogPath(stateRoot), {
...
async function logProxyUnsupportedPath(stateRoot, method, path) {
    await appendProxyJsonLine(proxyLogPath(stateRoot), {
...
function mergeReasoningMetadata(current, next) {
    return {
...
function reasoningMetadataKey(metadata) {
    return [
...
function isUpstreamCapacityError(body) {
    if (body.length === 0) {
        return false;
    }
...
async function isUpstreamCapacityErrorResponse(response) {
    if (isStreamContentType(response.headers.get("content-type") ?? "")) {
        return false;
    }
    const contentLength = Number(response.headers.get("content-length"));
    if (Number.isFinite(contentLength) && contentLength > PROXY_RESPONSE_INSPECTION_LIMIT_BYTES)
        return false;
    try {
...
function extractSessionId(requestJson, turnMetadata) {
...
function classifyProxyRoute(method, pathname) {
...
async function forwardRequest(request, upstream, body, signal) {
    ...
    const finalUrl = conversion?.needsConversion
        ? rewriteUpstreamUrl(new URL(rewriteUpstreamPath(requestUrl.pathname, conversion), requestUrl), upstream.baseURL)
...
class ProxyResponseWriteError extends Error {
    ...
    timeout;
    inspectionLimit;
    conversionFailureStage;
    constructor(message, status, responseBytes, timeout = null, inspectionLimit = false, conversionFailureStage = null) {
        super(message);
        this.status = status;
        this.responseBytes = responseBytes;
        this.timeout = timeout;
        this.inspectionLimit = inspectionLimit;
        this.conversionFailureStage = conversionFailureStage;
...
async function readProxyResponseBody(fetched, contentType, endpointClass, attemptStartedAtMs, callbacks) {
    ...
    const emitReasoningIfChanged = async (metadata) => {
...
async function proxyThroughActiveUpstreamPassthrough(request, upstream, body, gatewayRequestId, attemptRecords, callbacks) {
    const attemptState = {
        gatewayRequestId,
        attempts: 1,
        attemptRecords,
...
async function proxyThroughActiveUpstreamStatusRetry(request, initialUpstream, body, gatewayRequestId, attemptRecords, config, callbacks) {
    ...
    while (true) {
        ...
        try {
            ...
            const trigger = capacityMatched
                ? "capacity"
                : response.status === 429 ? "http_429" : "http_503";
            const retryAfter = parseRetryAfter(response.headers.get("retry-after"), Date.now(), capacityMatched ? 60_000 : config.total_window_ms);
            const retryAfterMs = retryAfter.kind === "missing_or_invalid" ? null : retryAfter.delayMs;
            const delayMs = retryAfter.kind === "missing_or_invalid"
...
function finishRequestConversionFailure(response, attemptState, upstream) {
...
function annotateProxyAttemptRoute(request, upstream, attempt) {
    ...
    if (conversion?.needsConversion && response.body) {
        ...
        if (contentType.includes("text/event-stream")) {
            ...
            upstreamStream.on("error", (error) => {
                if (!converter.endAfterUpstreamDisconnect())
                    converter.destroy(error);
            });
            const convertedStream = upstreamStream.pipe(converter);
            const headers = new Headers(response.headers);
            headers.delete("content-length");
            headers.delete("content-encoding");
            finalResponse = new Response(Readable.toWeb(convertedStream), {
                status: response.status,
...
function formatProxyStatusSummary(metrics) {
...
function formatProxyPolicySummary(records) {
...
function formatProxyStatusRetrySummary(records, config) {
    const totals = records.reduce((summary, record) => ({
        upstream_capacity: summary.upstream_capacity + record.retry_summary.upstream_capacity,
        http_429: summary.http_429 + record.retry_summary.http_429,
        http_503: summary.http_503 + record.retry_summary.http_503,
...
function proxyHistoryTableColumns(view) {
    if (view === "tokens")
        return PROXY_HISTORY_TOKEN_TABLE_COLUMNS;
    if (view === "cost")
        return PROXY_HISTORY_COST_TABLE_COLUMNS;
    return PROXY_HISTORY_OVERVIEW_TABLE_COLUMNS;
}
function renderProxyTable(columns, rows, indent, header = true) {
...
function proxyActiveRowCount(metrics) {
...
function formatProxyHistoryRows(metrics, records, count, now, view, sessionColorIndexes, priceCache) {
...
async function resolveProxyHistoryRecords(stateRoot, metrics, count, explicitHistory) {
    if (count <= 0) {
        return [];
    }
    if (explicitHistory) {
...
async function runProxyStatusWatch(options) {
    ...
    await runLiveView(() => renderProxyStatusLines({ ...options, watch: true, view, historyVisible }), {
...
export function proxyWatchKeyAction(key, view, historyVisible) {
    if (key === "q")
        return { view, historyVisible, action: "stop" };
    if (key === "t")
        return { view, historyVisible: !historyVisible, action: "render" };
    if (key !== "v")
        return { view, historyVisible, action: "none" };
    return {
...
async function forceShutdownProxyRuntime(options, health, activeRequestCount) {
...
async function buildProxyRestartPlan(options, force = false) {
    const state = await readProxyState(options.stateRoot);
    if (!state) {
        throw new Error(`proxy state file was not found: ${statePath(options.stateRoot)}`);
    }
...
async function proxyCancelRequest(state, target, init) {
...
async function buildProxyCancelPlan(options, target) {
    const state = await readProxyState(options.stateRoot);
    if (!state)
        throw new Error(`proxy state file was not found: ${statePath(options.stateRoot)}`);
    assertProxyHealthRuntime(await readProxyHealth(state));
    return { state, status: await proxyCancelRequest(state, target, { method: "GET" }) };
}
async function applyProxyCancelPlan(plan) {
    return proxyCancelRequest(plan.state, plan.status.target, {
        method: "POST",
    ...
    const server = createServer((req, res) => {
        ...
        void (async () => {
            ...
            try {
                    ...
                    if (route.endpoint === "reroute" || route.endpoint === "cancel") {
                        ...
                        if (route.endpoint === "cancel") {
                            if (method === "GET") {
                                ...
                                const sessionIds = new Set(matches.map((wait) => wait.session_id).filter((value) => value !== null));
                ...
                let upstream = null;
                let upstreamConversion = undefined;
                let attempts = 0;
                let responseBytes = 0;
                ...
                try {
                    ...
                    const passthroughCallbacks = {
                        ...
                        onStatusRetryWait: (wait) => {
                            ...
                            return () => {
                                if (statusRetryWaits.get(wait.requestId) === entry)
                                    statusRetryWaits.delete(wait.requestId);
                            };
                        },
                    ...
                    const guardedCallbacks = {
                        ...
                        onReasoningMetadata: async (metadata) => {
                            if (activeRecord.reasoning_tokens === metadata.reasoningTokens
                    ...
                    const upstreamEndpointClass = endpointClass === "responses"
                    ...
                    upstreamConversion = endpointClass === "responses"
                        ...
                        ? {
                            ...
                            toolContext: buildResponsesChatToolContext((requestJson ?? {})),
                        }
                        : undefined;
...
function parseProxyHistoryCount(rawCount) {
    if (!rawCount) {
